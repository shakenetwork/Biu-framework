...
def scan(target):
    for plugin in plugins:
        with open(plugin) as f:
            plugin = json.load(f)
        url = 'http://{}{}'.format(target, plugin['suffix'])
        try:
            ...
            for hit in plugin['hits']:
                if hit not in response:
                    ...
                    with open('reports/result_{}_{}.txt'.format(plugin['name'], today), 'a+')as res:
                        res.writelines(url + '\n')
                    break
